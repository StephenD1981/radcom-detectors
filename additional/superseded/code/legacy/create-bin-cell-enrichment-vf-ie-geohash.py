...
def loadInputData(input_path):
	for root, dir, file in os.walk(input_path):
		for filename in file:
			if ("DS_Store" not in filename) & ("SUCCESS" not in filename) & ("150" in filename):
...
def main():
	...
	try:
		print("\nRename columns..")
		grid_df.rename(columns = {'global_cell_id' : 'cilac', 'eventCount' : 'event_count'}, inplace = True)
	except:
	...
	try:
		print("\nCreate grid-cell aggregation..")
		print("\nPre grid-cell agg we have {} rows..".format(grid_df.shape[0]))
		grid_agg_df = grid_df[['grid', 'grid_cell', 'cilac', 'avg_rsrp', 'avg_rsrq', 'event_count']].groupby(['grid', 'grid_cell', 'cilac']).sum().reset_index()
		print("\ngrid-cell results in {} rows..".format(grid_agg_df.shape[0]))
	except:
	...
	try:
		...
		print("\ngrid-cell now contains {} rows..".format(grid_agg_df.shape[0]))
	except:
	...
	try:
		print("\nCreate GeoDataFrame..")
		grid_agg_df["geometry"] = grid_agg_df.apply(lambda x: "POINT (" + str(geohash.decode(x.grid)).split(", ")[1].replace(")", "") + " " + str(geohash.decode(x.grid)).split(", ")[0].replace("(", "") + ")", axis = 1)
	...
	try:
		print("\nSave to file..")
		grid_agg_gdf.to_csv("{}grid-cell-data-150m.csv".format(output_path), index = False)
	except:
